...
export default class SessionPage extends React.Component {
  constructor() {
    super();
    this.state = { isLoading: true, error: false, refreshing: false,amount:0 }
  }
  _onRefresh() {
  ...
  generaterows() {
    for (i = 0; i < this.state.dataSourceAPI.rowcount; i++) {
      return(
  ...
  generateseats(){
    for (j = 9; j < this.state.dataSourceAPI.columncount; i++) {
      return(
        <TouchableOpacity style={styles.seat} activeOpacity={1} >
