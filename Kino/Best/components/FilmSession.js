...
export default class FilmSession extends React.Component {
  ...
  componentDidMount() {
    var now = new Date();
    now.setDate(now.getDate() + this.state.idday)
    var formated_date = now.toISOString();
    return fetch(global.ip + '/api/session/?film=' + this.props.link + '&date=' + formated_date, { timeout: 500, follow: 0 })
          ...
          <View style={styles.row}>
            <View style={styles.column} >
              <Picker
                ...
                onValueChange={(itemValue, itemIndex) => { this.setState({ day: itemValue, idday: itemIndex }); this.valuechange(itemIndex) }}>
